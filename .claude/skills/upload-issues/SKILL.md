...
or more `MC-xxx` issues by `/generate-issues`. If the file does not exist
...
- `Title`
- `Size` (S, M, L)
- `Area` (the component: `panels`, `fs`, `fileops`, `console`, `viewer`,
  `editor`, `widgets`, `keymap`, `theme`, `state`, `platform`, `ai`,
  `claude-code`, `tests`)
- `Phase` (the roadmap phase it implements, e.g. `v0.2`)
- `Dependencies` (list of MC-xxx IDs)

Then parse each **detailed issue section** (heading with MC-xxx) to
...
roadmap.md and the far-spec F-items it references).
...
Version titles: **v0 — MVP ("it feels like Far")**, **v1 — First public
release** (from [spec/roadmap.md](../../../spec/roadmap.md)).

```bash
# Version label
gh label create "v0::version:0" --color "0E8A16" --description "Version v0 — MVP (it feels like Far)" 2>/dev/null || true
...
gh label create "v0::area:panels"      --color "1D76DB" 2>/dev/null || true
gh label create "v0::area:fileops"     --color "D93F0B" 2>/dev/null || true
gh label create "v0::area:console"     --color "6F42C1" 2>/dev/null || true
gh label create "v0::area:widgets"     --color "FBCA04" 2>/dev/null || true
gh label create "v0::area:ai"          --color "0E8A16" 2>/dev/null || true
# ... fs / viewer / editor / keymap / theme / state / platform / claude-code / tests as needed
```
...
**Size:** {S/M/L}
**Version:** v{n}
**Area:** {panels/fs/fileops/console/viewer/editor/widgets/keymap/theme/state/platform/ai/claude-code/tests}
...
|-------|----------|-------|-------|--------|-----|
| MC-009 | #12 | ... | v0.1 | v0::version:0, v0::size:S, v0::area:panels | {url} |
