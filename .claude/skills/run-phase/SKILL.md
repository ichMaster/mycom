...
1. **`/generate-issues <phase>`** — decompose roadmap §`vA.B` into
   `spec/roadmap/implementation/v{A.B}-issues.md`.
2. **`/upload-issues <file>`** — push that file to GitHub as `MC-xxx` issues
...
roadmap one-phase-per-release model).
...
- `/run-phase 0.2` — run v0.2 (dialog kit, Far theme, key bar) through generate → upload → execute
- `/run-phase v0.4` — run v0.4 (file operations)
...
2. **Preflight checks** (fail fast — report and stop if any fails):
   ...
   - Read [spec/roadmap.md](../../../spec/roadmap.md) §`vA.B` — confirm the
     phase exists and read its Goal/Tasks/DoD.
3. **Guard against redoing a finished phase.** Check `VERSION` / `git tag` and
   ...
   - If `A.B.0` is already tagged (for v0.1: `0.1.1` — the retrofit release),
     or the phase's issues are all **closed**, the phase is done — **stop and
     ask** whether the user really means this phase (they likely meant the next
     one). Do not silently regenerate a completed phase.
...
1. **Generate** — invoke `generate-issues` with `<phase>`.
   ...
   - Success criterion: `spec/roadmap/implementation/v{A.B}-issues.md` exists
     and parses (has an Issues Summary Table with `MC-xxx` rows).
...
2. **Upload** — invoke `upload-issues` with the issues file
   ...
   - Success criterion: every `MC-xxx` in the issues file maps to an open
...
- **Stage 1:** issues file path + issue count + `MC-xxx` id range.
- **Stage 2:** GitHub issues created (MC-xxx → #), report path.
...
- **Next step:** the phase is at its DoD but **unreleased** — suggest the
  ...
  (For v0.1 — the retrofit on the pre-pivot `0.1.0` — suggest `0.1.1`.)
